...
- **流式 TF + IDF 缓存** — 查询路径按存储版本缓存 IDF（词逆频率）；命中时单次流式遍历 20k 条目仅需 ~3 ms（5k 文件） / ~0.6 ms（1k 文件），零中间对象。只有**真正脏了**的写入才递增版本号并清空缓存——无变更时 `save()` 在碰盘前直接返回，因此 15 秒一轮的 watch 轮询不会把查询刚建好的 IDF 缓存清掉。
...
- **开销可忽略** — 纯进程内操作；冷启动 <100 ms（5k 文件），典型项目查询中位数 2–3 ms（p99 < 7 ms）；瓶颈在 PDF 解析与磁盘 I/O，插件本身的打分开销不阻塞。
...
> 合成基准：生成代码（~4–5 符号/文件），Node 24，Linux 文件系统，SSD。测量纯索引开销，不含 LLM 调用。query_memory 基准使用 IDF 缓存 + 预计算 searchText；写入后的首次查询会重建 IDF（~120–150 ms，随规模变化；40k 条目实测约 122 ms），后续查询命中缓存。
...
```
冷索引     249 ms   （read+hash 8 ms · extract 224 ms · commit 15 ms）
存储       1.10 MB · 538 bytes/条目 · 冷加载 4.3 ms
热查询     p50 0.75 ms · p95 1.06 ms          （2,141 条目）
单文件重索引  p50 0.31 ms
```

两个我们宁可自己说清楚的坑：`read+hash` 受操作系统页缓存影响（同一个语料第一遍比第二遍慢，报数时请说明是第几遍）；**真实项目比上面的合成基准慢**——在一个大型 TypeScript 仓库的 3,000 文件切片上（15,594 条目）热查询 p50 为 7.5 ms，因为真实声明文本比生成出来的桩代码长得多。带 `--queries 你的查询集.json` 可以在你自己的项目上跑同一套标注集方法（hit@5 / hit@10 / MRR）。
