...
### Changed (index-time stays zero-LLM; retrieval terms split from injected summary)

- **Index-time LLM removed by construction; the zero-LLM 铁律 holds.** `de-TODO.md` (three iron rules, 2026-08-31) says `零依赖、零后台、零 LLM 索引期 —— 违反即废` and lists `LLM 索引期` under 彻底不做的事. The doc pipeline had carried an LLM path since its first commit that never once ran (`src/llm.js` called `llm.stream` without the host-required `provider`/`model`, so `NO_ADAPTER` was thrown and swallowed) — meaning truncation was the de-facto behaviour. Rather than wire the LLM up (which an earlier pass proposed as "D4"), `buildDocEntries` now takes **no `llm` parameter at all**, so index-time cannot reach a model even by accident. `src/doc-pipeline.js`, `src/watch.js`, `src/lazy.js`, `index_doc` and `index_repo` no longer resolve or pass a route.
- **Retrieval terms are now separate from the injected summary (方案 A).** The old shape made the whole chunk's searchable body the ≤300-char `summary` (`util/search.js` `weightedFieldText`), so a 3000-char chunk had ~90% of its text invisible to `query_memory`. Each doc entry now also carries `terms` — a bounded (≤160), deterministic, stop-word-filtered literal term set covering the **entire chunk** (`src/doc-index.js`, built from the existing `tokenizeRaw`). `summary` stays short for the injection budget; `terms` is search-only.
  - Measured on this repository (20 doc files, 223 chunks): distinct retrievable chunk terms go from 8,143 (47.7%, first-300-chars only) to 17,055 (100%) — **52.3% of previously invisible terms recovered**, with **zero** LLM calls. `no-hit` cases for content sitting past char 300 now return hits.
...
- **Recall-time opt-in LLM is routed correctly instead of silently failing.** `config.llmQueryExpansion` (default off) and `reflection.enabled` (default off) are the only remaining model calls — both sanctioned as 召回期按需可选. They now go through `src/llm-route.js` (config override → session `requestHeader().config` → agent options → last-seen `request/header` route) and `chatText` passes `provider`/`model`, which the host requires. Missing routes fall back explicitly and leave a visible one-time `degraded` record rather than being swallowed.
- **Rule-based keywords replace the LLM-derived 5–10 keywords** (`extractKeywords`: title-weighted top terms), so `keywords` is deterministic and reproducible even though the intended bilingual expansion is no longer possible at index time.
- **Tests:** `test/doc-index.test.mjs` (new, 7 checks) pins summary ≤300 / terms covering past char 300 / BM25 hitting a post-300 term / zero LLM calls at index time even with a routed `exec` / deterministic bounded `extractTerms` / the automatic back-fill. `test/llm-route.test.mjs` is trimmed to the recall-time routing contract (4 checks). `test/run-test.mjs` now asserts watch indexing makes **zero** LLM calls and that doc entries are structural. Suite 227 checks.
- **Decision provenance:** the earlier "D4" fix (commit `d6c3592`) is superseded, not extended — the missing `provider`/`model` was reclassified from "bug" to "the 铁律 being honoured the hard way", and the coverage problem it exposed is fixed without a model.

### Files
- src/doc-index.js (new), src/doc-pipeline.js, src/llm.js, src/llm-route.js (new), src/util/text.js, src/util/search.js, src/link.js, src/index.js, src/watch.js, src/lazy.js, src/reflection-pipeline.js, src/tools/index-doc.js, src/tools/index-repo.js, src/tools/query-memory.js, src/tools/task-tools.js, src/commands/task-actions.js, test/doc-index.test.mjs (new), test/llm-route.test.mjs, test/run-test.mjs, test/reflection-pipeline.test.mjs, package.json, CHANGELOG.md
